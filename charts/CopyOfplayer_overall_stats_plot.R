...
ItemChampGroup <- inner_join(read.csv("./db/playerMatchStats.csv"),filterForMatches,by = c('player_id','position','champion_name'))
...
myStatsPositionReactive <- function(summoner_name,summoner_position,id1,type,compare = "brak"){
  myStatsPosition1 <- ItemChampGroup %>% filter(player_id == as.vector(summoner %>% filter(name %in% summoner_name) %>% select(puuid)),position==summoner_position)
  if (type=="Density") {
    if (id1=="All") {
      ...
      if (id1=='All'){
        myStatsPosition2temp <- ItemChampGroup %>% filter(player_id == as.vector(summoner %>% filter(name %in% summoner_name) %>% select(puuid)),position==summoner_position)
      } else {
        myStatsPosition2temp <- ItemChampGroup %>% filter(player_id == as.vector(summoner %>% filter(name %in% summoner_name) %>% select(puuid)),position==summoner_position,champion_name==id1)
      }
...
#   myStatsPosition2 <- df_csv
#   if (nrow(myStatsPosition2)!=0) {
#     if (type=="Density") {
#       if (stat=="DmgPerDeatch") {
...
univPlot <- function(summoner_name,summoner_position,id1,type,stat,compare = "brak"){
  myStatsPosition2 <- myStatsPositionReactive(summoner_name,summoner_position,id1,type,compare)
  if (nrow(myStatsPosition2)!=0) {
    if (type=="Density") {
      if (stat=="DmgPerDeath") {
        gg <- ggplot(data = myStatsPosition2 %>% mutate(deaths = ifelse(deaths==0,1,deaths)),aes(x = total_damage_dealt_to_champions/deaths)) +
...
server <- function(input, output) {
  ...
  output$dynamicInput <- renderUI({
    myStatsPosition <- ItemChampGroup %>% filter(player_id == as.vector(summoner %>% filter(name %in% input$summoner) %>% select(puuid)),position==input$Position)
  ...
  #   myStatsPosition1 <- ItemChampGroup %>% filter(player_id == as.vector(summoner %>% filter(name %in% input$summoner) %>% select(puuid)),position==input$Position)
  #   if (input$id1=="All") {
  #     myStatsPosition2 <- myStatsPosition1 %>%
  ...
  #     if (input$type=="Density") {
  #       myStatsPosition2 <- ItemChampGroup %>%
  #         filter(player_id == as.vector(summoner %>% filter(name %in% input$summoner) %>% select(puuid)),position==input$Position) %>% mutate(compare = ifelse(champion_name==input$id1,input$id1,input$compare))
  ...
  #       }
  #     } else {
  #       myStatsPosition2 <- ItemChampGroup %>% filter(player_id == as.vector(summoner %>% filter(name %in% input$summoner) %>% select(puuid)),position==input$Position,champion_name==input$id1)
  #     } 
  #   }
  #   return(myStatsPosition2)
  # })
  ...
  output$DmgPerDeath <- renderPlotly({
    if (input$type=='Density') {
      univPlot(input$summoner,input$Position,input$id1,input$type,"DmgPerDeath",input$compare)
    } else {
      univPlot(input$summoner,input$Position,input$id1,input$type,"DmgPerDeath")
    }
  })
  
  output$kill_participation <- renderPlotly({
    if (input$type=='Density') {
      univPlot(input$summoner,input$Position,input$id1,input$type,"kill_participation",input$compare)
    } else {
      univPlot(input$summoner,input$Position,input$id1,input$type,"kill_participation")
    }
  })
  
  output$MinionsPerMinute <- renderPlotly({
    if (input$type=='Density') {
      univPlot(input$summoner,input$Position,input$id1,input$type,"MinionsPerMinute",input$compare)
    } else {
      univPlot(input$summoner,input$Position,input$id1,input$type,"MinionsPerMinute")
    }
  })
  
  output$Kda <- renderPlotly({
    if (input$type=='Density') {
      univPlot(input$summoner,input$Position,input$id1,input$type,"kda",input$compare)
    } else {
      univPlot(input$summoner,input$Position,input$id1,input$type,"kda")
    }
  })
}
